...
use crate::{application, presentation::http::{AppState, auth::Claims, error::ApiError}};
...
pub async fn login(
    ...
    let token = encode(&Header::default(), &claims, &EncodingKey::from_secret("some-secret".as_bytes())).expect("failed to create jwt");
    Ok(Json::from(LoginResponse{
        jwt: token
    }))
}